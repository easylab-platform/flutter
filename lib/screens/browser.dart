...
///
/// Empty query: org → repo → branch tree. Org rows open [OrgDetailPage],
/// repo rows open the repo detail screen, branch rows pick/adopt a session.
/// Long-press exposes the destructive actions that used to live on the chat
/// sidebar tree (delete org/repo, fork branch).
///
/// Non-empty query: matching sessions plus matching branches.
...
class _BrowserPageState extends State<BrowserPage> {
  ...
  List<Session> _hits = [];
  List<(OrgNode, RepoNode, BranchNode)> _bmHits = [];
  ...
  Future<void> _forkDialog(String sessionId) async {
    ...
    if (r != null && r.trim().isNotEmpty) {
      final branch = r.trim();
      if (store.existingBranchs.contains(branch)) {
        if (!mounted) return;
        ScaffoldMessenger.of(context)
  ...
  /// Non-empty query: matching sessions first, then matching branches.
  ...
  /// Empty-query view: the full org → repo → branch tree.
    ...
    return Column(
      ...
      children: [
        ...
        for (final repo in org.repos) ...[
          _repoRow(org, repo),
          for (final bm in repo.branches)
            _branchRow(org, repo, bm, indent: true),
        ],
  ...
  Widget _branchRow(OrgNode org, RepoNode repo, BranchNode bm,
      {bool indent = false}) {
    final colors = colorsOf(context);
    final text = textOf(context);
    final bound = bm.session != null;
    return InkWell(
      onTap: () => _openBranch(org.org, repo.repo, bm.branch, bm.session?.sessionId),
      onLongPress: bound
